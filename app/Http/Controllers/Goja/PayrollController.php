...
use App\Models\Dian\PayrollSettlementGoja;
...
class PayrollController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        return Inertia::render('Applications/Goja/Index', [
            'years' => $years,
        ]);
    }
    ...
    {
        $settlement = PayrollSettlementGoja::where('AÑO', '=', $r->year)
            ->where('MES', '=', $r->month)
            ...
            ->get()->map(function ($row) use ($r) {
                $row->current_document = PayrollDocument::where('entity', '=', 'GOJA')
        ...
        try {
            foreach ($request->employees as $employee) {
                if ($request->type_operation === 'payroll') {
                    ...
                    $params = $this->generateParamsDestroy($employee['document_reference'], 'GOJA');
                }
                if (! $params['status']) {
                    continue;
                }
            ...
            $this->sendDIAN($request->year, $request->month, $request->start_period, $request->end_period, 'GOJA');
