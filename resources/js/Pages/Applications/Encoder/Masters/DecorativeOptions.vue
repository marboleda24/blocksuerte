<template>
    <div>
        <Head title="Opciones Decorativas"/>
                ...
                <template #content>
                    <div class="p-2">
                        ...
                        <div class="mb-4">
                            ...
                            <input v-model.trim="form.abbreviation" type="text" class="form-control"
        ...
        return {
            form: {
                ...
                name: {
                    ...
                    isUnique: helpers.withMessage('Esta opción decorativa ya se encuentra registrada', withAsync(async (value) => {
                        if(value) {
                            if (source) source.cancel();
                            source = CancelToken.source();
                            try {
                                return await axios.get(route('decorative-options.validate-name', [value]), {
    ...
    methods: {
        ...
        reset: function () {
            this.form = {
                ...
                this.title = 'Crear opcion decorativa'
        },

        save(data) {
            axios.post(route('decorative-options.store', data)).then(res => {
                this.closeModal();
                this.tableData = res.data;

                this.$swal({
                    title: '¡Éxito!',
                    text: "Opción decorativa creado con éxito!",
                    icon: 'success',
        ...
        deleteRow(row) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(route('decorative-options.destroy', row)).then(resp => {
        ...
        update(row) {
            axios.put(route('decorative-options.update', row.code), row).then(resp => {
                this.tableData = resp.data;
                this.closeModal();
                this.$swal({
                    title: '¡Éxito!',
        ...
        generate_code() {
            if (!this.editMode) {
                axios.get(route('decorative-options.get-latest-code')).then(resp => {
