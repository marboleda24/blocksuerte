            ...
            <div class="p-5">
                <div class="grid grid-cols-3 gap-6">
                    <div class="mb-2">
                        ...
                        <select class="form-select" required autofocus  :class="{ 'border-danger': v$.form.year.$error }" v-model="form.year" @change="getMonths(form.year)">
                    ...
                    <div class="mb-2">
                        ...
                        <select class="form-select" :class="{ 'border-danger': v$.form.month.$error }" v-model="form.month" @change="getPeriods(form.year, form.month)">
                            <option value="" disabled selected>Seleccione...</option>
                    ...
                    <div class="mb-2">
                        ...
                        <select class="form-select" :class="{ 'border-danger': v$.form.period.$error }" v-model="form.period">
                            <option value="" disabled selected>Seleccione...</option>
                            <option v-for="period in periods" :value="period.period">{{ period.period+' - '+ period.notes }}</option>
                        </select>
                    ...
                    <div class="mb-2" v-if="form.type === 'unique'">
                        <label class="flex flex-col sm:flex-row">
                            Empleado
                            <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-gray-600">
                                Obligatorio*
                        ...
                        <autocomplete
                            url="/goja/human-resource/search-employee"
                            show-field="nombres"
                ...
                <div class="flex-row justify-center text-center mt-5">
                    <button class="btn btn-primary" @click="sendMail(form)">
                        {{ form.type === 'unique' ? 'Enviar Comprobante' : 'Enviar Comprobantes' }}
                    </button>

                    <template v-if="form.type === 'unique'">
                        <button class="btn btn-secondary ml-4" @click="downloadFile(form)">
                            Descargar
                        </button>
                    </template>
                </div>
...
export default {
    ...
    methods: {
        sendMail(data){
            ...
            if (!this.v$.$invalid) {
                if (data.type === 'global'){
                    ...
                    axios.post(route('goja.human-resource.proof-payment.send-mail'), data).then(resp => {
                        this.$swal({
                            icon: 'success',
                            title: '¡Éxito!',
        ...
        getMonths(year){
            ...
            axios.get(route('goja.human-resource.proof-payment.get-months'), {
                params: {
                    year: year
                }
            }).then(resp => {
        ...
        getPeriods(year, month){
            ...
            axios.get(route('goja.human-resource.proof-payment.get-periods'), {
                params: {
                    year: year,
                    month: month
                }
        ...
        downloadFile(data){
            ...
            if (!this.v$.$invalid) {
                ...
                axios.post(route('goja.human-resource.proof-payment.download'), data, {
                    responseType: 'blob'
                }).then(resp => {
