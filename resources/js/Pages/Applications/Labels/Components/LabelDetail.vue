<template>
    <div class="intro-y box mt-4" v-if="Object.keys(data).length > 0">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60 dark:border-darkmode-400">
            <h2 class="font-medium text-base mr-auto">Etiqueta Detalle</h2>
        </div>
        <div class="p-5">
            <div>
                <div class="p-2 text-base font-medium font-bold uppercase">
                    Detalle de Orden
                </div>

                <div
                    class="grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 mt-0 p-5 uppercase border-2 rounded-lg">
                    <div class="form-inline text-base">
                        <label class="form-label font-bold">Orden de Producción:</label>
                        <code class="form-label">{{ data.op }}</code>
                    </div>
                    <div class="form-inline text-base">
                        <label class="form-label font-bold">Producto:</label>
                        <code class="form-label">{{ `${data.product_code} - ${data.product_description}` }}</code>
                    </div>
                    <div class="form-inline text-base">
                        <label class="form-label font-bold">Pedido:</label>
                        <code class="form-label">{{ data.order }}</code>
                    </div>
                    <div class="form-inline text-base">
                        <label class="form-label font-bold">Lote:</label>
                        <label class="form-label">{{ data.lot }}</label>
                    </div>
                    <div class="form-inline text-base">
                        <label class="form-label font-bold">Código Arte:</label>
                        <label class="form-label">{{ data.art }}</label>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        generateLabel() {
            ...
            result.push('^FR^FD' + this.data.product_code + '^FS\n')
            result.push('^CF0,25\n')
            result.push('^FO45,35^FD' + this.data.product_description + '^FS\n')
            result.push('^FO45,110^FD' + this.data.product_code + '^FS\n')
            result.push('^FO45,135^FDR: ' + this.form.reviewer + '^FS\n')
            result.push('^FO210,135^FDE: ' + this.form.packer + '^FS\n')
            if (this.data?.lot?.length > 25){
                result.push('^FO45,160^FDLOTE: ' + this.data.lot.slice(0, 24) + '^FS\n')
                result.push('^FO45,180^FD' + this.data.lot.slice(24) + '^FS\n')

            }else {
                result.push('^FO45,160^FDLOTE: ' + this.data.lot + '^FS\n')
            }
            result.push('^FO460,56^FDOP: ' + this.data.op + '^FS\n')
            result.push('^FO460,80^FDARTE: ' + this.data.art + '^FS\n')
            result.push('^FO460,105^FDCANTIDAD: ' + this.form.package_quantity + ' unidades^FS\n')
            result.push('^FO460,130^FDOV: ' + this.data.order + '^FS\n')
