<template>
    <div>
        <Head title="Pedidos Pendientes (Producción)"/>

        <portal to="application-title">
            Pedidos Pendientes (Producción)
        </portal>
        ...
        <div>
            ...
            <show-order-modal :info-modal="infoModal" :is-open="isOpen" :mark-buttons="true" @mark="mark_order" @close="closeModal"/>
        </div>
    </div>
</template>
...
export default {
            ...
            table_pending: {
                ...
                options: {
                    ...
                    templates: {
                        ...
                        mark(h, row) {
                            if (row.mark === 'moon') {
                                return <font-awesome-icon icon="moon" class="text-yellow-400"/>
                            } else if (row.mark === 'star') {
                                return <font-awesome-icon icon="star" class="text-yellow-400"/>
                            }
                        },
                        type(h, row) {
                            switch (row.type) {
    ...
    methods: {
        finalize(row) {
            if (row.destiny === 'P' && row.type !== 'point_of_sale') {
                this.$swal({
            ...
            } else if (row.destiny === 'P' && row.type === 'point_of_sale') {
                this.$swal({
                    title: '¡Selecciona una razon!',
                    text: 'El pedio seleccionado es para un punto de venta, por favor seleccione el almacen',
                    icon: 'info',
                    input: 'select',
                    inputOptions: {
                        '30': 'Itagui',
                ...
                this.$swal({
                    icon: 'error',
                    title: '¡Ups!',
                    text: 'Hubo un error solicitando el tipo de pedido.',
                    confirmButtonText: 'Aceptar',
                });
            }
        ...
        refuse(row) {
            this.$swal({
                ...
                allowEscapeKey: false,
                    ...
                    axios.post(route('orders.production.refuse'), {
                        'id': row,
                        'justify': inputValue.value
                    }).then(res => {
                        this.tableData = res.data.orders
        ...
        send_wallet(id) {
            this.$swal({
                ...
                allowOutsideClick: false,
                allowEscapeKey: false,
                        title: 'Enviando a bodega...',
                        text: 'Este proceso puede tardar unos segundos.',
                    });

                    axios.post(route('orders.production.send-wallet'), {
                        id: id,
                        justify: inputValue.value
                    }).then(res => {
                        this.tableData = res.data.orders
                        ...
                        this.$swal({
                            title: '¡Éxito!',
                            text: "¡El pedido ha sido enviado a bodega!",
                            icon: 'success',
                            confirmButtonText: 'Aceptar'
                        });
                    }).catch(err => {
        ...
        mark_order(id, icon) {
            axios.post(route('orders.mark-order'), {
                id: id,
                icon: icon
            }).then(resp => {
                this.infoModal.mark = icon;
