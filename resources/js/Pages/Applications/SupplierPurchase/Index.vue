...
export default {
        ...
        return {
            table: {
                ...
                options: {
                    ...
                    filterAlgorithm: {
                        prefix(row, query) {
                            return (row.document_information.Prefix.toLowerCase()).includes(query)
                        },
                        consecutive(row, query) {
                            return (row.document_information.ID.replace(row.document_information.Prefix, '').toLowerCase()).includes(query)
                        },
                        supplier_name(row, query){
                            return (row.supplier.Name.toLowerCase()).includes(query)
                        },
                        supplier_nit(row, query){
                            return (row.supplier.CompanyID.toLowerCase()).includes(query)
                        }
                    },
    ...
    methods: {
        uploadFile() {
            this.$swal({
                ...
                if (inputValue.value) {
                    ...
                    axios.post(route('supplier-purchases.import'), formData, {
                        headers: {
        ...
        async acuse(id) {
            ...
            this.classifications.map(elem => classifications[elem] = elem);
            this.work_centers.map(elem => work_centers[elem] = elem);
            ...
            const swalQueueStep = this.$swal.mixin({
                ...
                inputValidator: (value) => {
                    return new Promise((resolve) => {
                        if (value === 'Seleccione…' || value === '') {
                            resolve('Este campo es obligatorio')
                        } else {
                            resolve()
                        }
                    })
            ...
            for (currentStep = 0; currentStep < steps.length;) {
                ...
                if (currentStep === 0) {
                    ...
                    result = await swalQueueStep.fire({
                        ...
                        text: 'Area responsable de gestionar este documento',
                        input: 'select',
                        inputOptions: work_centers,
                        showCancelButton: currentStep > 0,
                        currentProgressStep: currentStep,
                        inputAttributes: {
                            required: true
                        },
                        confirmButtonText: 'Notificar recepción',
                    })
                }
                ...
                if (result.value) {
                    switch (currentStep) {
                        case 0:
                            values.classification = result.value;
                            break;
                        case 1:
                            values.work_center = result.value;
                            break;
                    }
                    currentStep++
            ...
            if (currentStep === steps.length) {
                this.$swal({
                    ...
                    allowOutsideClick: false,
                    allowEscapeKey: false,
                    title: 'Notificando recepción…',
                    text: 'Este proceso puede tardar unos segundos.',
                });

                axios.post(route('supplier-purchases.notify-reception'), {
                    id: id,
                    state: 1,
                    ...
                    this.$swal({
                        title: '¡Éxito!',
                        text: "Recepción de documento notificado con éxito",
                        icon: 'success',
                        confirmButtonText: 'Aceptar'
                    });
                }).catch(err => {
    ...
    computed: {
        ...
        notify_rows(){
            return this.table.data.filter(row => row.dian_state === '030').sort((a, b)=>{
