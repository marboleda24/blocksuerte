        ...
        <jet-dialog-modal :show="isOpen" @close="closeModal" max-width=lg>
            ...
            <template #content>
                <div class="input-form mb-4">
                    ...
                    <input type="text"
                           class="form-control"
                           :class="{ 'border-danger': v$.form.address1.$error }"
                           v-model="form.address1">

                    <template v-if="v$.form.address1.$error">
                        <ul class="mt-1">
                            <li class="text-danger" v-for="(error, index) of v$.form.address1.$errors"
                                :key="index">
                                {{ error.$message }}
                            </li>
                        </ul>
    ...
    methods: {
        ...
        update(form) {
            ...
            if (!this.v$.form.$invalid) {
                ...
                axios.post(route('customer.update-information.address-1'), form).then(resp => {
                    this.closeModal()
                    this.$emit('success', {
                        property: 'DIRECCION', value: resp.data
                    })
