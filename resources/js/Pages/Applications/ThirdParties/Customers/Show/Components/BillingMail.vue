        ...
        <jet-dialog-modal :show="isOpen" @close="closeModal" max-width=lg>
            ...
            <template #content>
                <div class="input-form mb-4">
                    ...
                    <input type="email"
                           class="form-control"
                           :class="{ 'border-danger': v$.form.billing_email.$error }"
                           v-model="form.billing_email">

                    <template v-if="v$.form.billing_email.$error">
                        <ul class="mt-1">
                            <li class="text-danger" v-for="(error, index) of v$.form.billing_email.$errors"
                                :key="index">
                                {{ error.$message }}
                            </li>
                        </ul>
    ...
    methods: {
        ...
        update(form) {
            ...
            if (!this.v$.form.$invalid) {
                ...
                axios.post(route('customer.update-information.billing-mail'), form).then(resp => {
                    this.closeModal()
                    this.$emit('success', {
                        property: 'CORREO_FE', value: resp.data
                    })

                    this.$swal({
                        icon: 'success',
                        title: '¡Éxito!',
                        text: 'Email de facturación actualizado con éxito.',
