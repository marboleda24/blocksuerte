        ...
        <jet-dialog-modal :show="isOpen" @close="closeModal" max-width=lg>
            ...
            <template #content>
                <div class="input-form mb-4">
                    ...
                    <input type="text"
                           class="form-control"
                           :class="{ 'border-danger': v$.form.business_name.$error }"
                           v-model="form.business_name" v-uppercase>

                    <template v-if="v$.form.business_name.$error">
                        <ul class="mt-1">
                            <li class="text-danger" v-for="(error, index) of v$.form.business_name.$errors"
                                :key="index">
                                {{ error.$message }}
                            </li>
                        </ul>
    ...
    methods: {
        ...
        update(form) {
            ...
            if (!this.v$.form.$invalid) {
                ...
                axios.post(route('customer.update-information.business-name'), form).then(resp => {
                    this.closeModal()
                    this.$emit('success', {
                        property: 'RAZON_SOCIAL', value: resp.data
                    })

                    this.$swal({
                        icon: 'success',
                        title: '¡Éxito!',
                        text: 'Razon social actualizada con éxito.',
