            ...
            <template #content>
                <div class="input-form mb-4">
                    ...
                    <select  class="form-select"
                             :class="{ 'border-danger': v$.form.customer_type.$error }"
                             v-model="form.customer_type">
                        <option :value="type.CUSTYP_62.trim()"
                                v-for="type in types">
                            {{ `${type.CUSTYP_62.trim()} - ${type.DESC_62.trim()}` }}
                        </option>
                    </select>

                    <template v-if="v$.form.customer_type.$error">
                        <ul class="mt-1">
                            <li class="text-danger" v-for="(error, index) of v$.form.customer_type.$errors"
                                :key="index">
                                {{ error.$message }}
                            </li>
                        </ul>
    ...
    methods: {
        ...
        update(form) {
            ...
            if (!this.v$.form.$invalid){
                ...
                axios.post(route('customer.update-information.customer-type'), form).then(resp => {
                    this.closeModal()
                    this.$emit('success', {
                        property:'TIPO_CLIENTE', value: resp.data
                    })
        ...
        getName(code){
            let coincidence = this.types.find(elem => elem.CUSTYP_62.trim() === code)

            return coincidence
                ? `${coincidence.CUSTYP_62} - ${coincidence.DESC_62.trim()}`
                : code
        }
    },
    mounted() {
