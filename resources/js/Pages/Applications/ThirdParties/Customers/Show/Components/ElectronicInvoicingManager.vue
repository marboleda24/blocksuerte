<template>
    <div>
            ...
            <div class="flex flex-col lg:flex-row items-center p-5">
                ...
                <div class="lg:ml-2 lg:mr-auto text-center lg:text-left mt-3 lg:mt-0">
                    ...
                    <div class="text-slate-500 text-xs mt-0.5">
                        {{ electronic_invoicing_manager?.length > 0 ? electronic_invoicing_manager : 'SIN REGISTRAR' }}
                    </div>
                </div>
                <div class="flex mt-4 lg:mt-0 text-center text-slate-500 ml-1">
                    ...
                    <button class="btn btn-secondary ml-2"
                            @click="trash"
                            v-if="electronic_invoicing_manager">
        ...
        <jet-dialog-modal :show="isOpen" @close="closeModal" max-width=lg>
            <template #title>
                Actualizar Responsable Facturación Electronica
            </template>
            ...
            <template #content>
                <div class="input-form mb-4">
                    ...
                    <input type="text"
                           class="form-control"
                           :class="{ 'border-danger': v$.form.electronic_invoicing_manager.$error }"
                           v-model="form.electronic_invoicing_manager" v-uppercase>

                    <template v-if="v$.form.electronic_invoicing_manager.$error">
                        <ul class="mt-1">
                            <li class="text-danger"
                                v-for="(error, index) of v$.form.electronic_invoicing_manager.$errors"
                                :key="index">
                                {{ error.$message }}
                            </li>
                        </ul>
...
export default {
    ...
    props: {
        electronic_invoicing_manager: String,
    ...
    data() {
        return {
            form: {
                ...
                electronic_invoicing_manager: this.electronic_invoicing_manager,
                justify: ''
            },
            isOpen: false
        }
    ...
    methods: {
        ...
        update(form) {
            ...
            if (!this.v$.form.$invalid) {
                ...
                axios.post(route('customer.update-information.electronic-invoicing-manager'), form).then(resp => {
                    this.closeModal()
                    this.$emit('success', {
                        property: 'RESPONSABLE_FE', value: resp.data
                    })

                    this.$swal({
                        icon: 'success',
                        title: '¡Éxito!',
                        text: 'Responsable Facturación Electronica actualizado con éxito.',
        ...
        trash() {
            this.$swal({
                title: '¿Eliminar Responsable Facturación Electronica?',
                ...
                if (inputValue.value) {
                    ...
                    axios.post(route('customer.update-information.electronic-invoicing-manager'), {
                        customer_code: this.customer_code,
                        electronic_invoicing_manager: '',
                        ...
                        this.closeModal()
                        this.$emit('success', {
                            property: 'RESPONSABLE_FE', value: ''
                        })

                        this.$swal({
                            icon: 'success',
                            title: '¡Éxito!',
                            text: 'Responsable Facturación Electronica eliminado con éxito.',
                            confirmButtonText: 'Aceptar',
                            timerProgressBar: true,
                            timer: 6000
                        });
