            ...
            <div class="flex flex-col lg:flex-row items-center p-5">
                ...
                <div class="lg:ml-2 lg:mr-auto text-center lg:text-left mt-3 lg:mt-0">
                    ...
                    <div class="text-slate-500 text-xs mt-0.5">
                        {{ gravado ? (gravado === 'Y' ? 'SI' : 'NO') : 'SIN REGISTRO' }}
                    </div>
                </div>
            ...
            <template #content>
                <div class="input-form mb-4">
                    ...
                    <select  class="form-select"
                             :class="{ 'border-danger': v$.form.gravado.$error }"
                             v-model="form.gravado">
                        <option :value="null">Seleccione…</option>
                        <option value="Y">SI</option>
                        <option value="N">NO</option>
                    </select>

                    <template v-if="v$.form.gravado.$error">
                        <ul class="mt-1">
                            <li class="text-danger" v-for="(error, index) of v$.form.gravado.$errors"
                                :key="index">
                                {{ error.$message }}
                            </li>
                        </ul>
    ...
    methods: {
        ...
        update(form) {
            ...
            if (!this.v$.form.$invalid){
                ...
                axios.post(route('customer.update-information.gravado'), form).then(resp => {
                    this.closeModal()
                    this.$emit('success', {
                        property:'GRABADO',
                        value: resp.data
                    ...
                    this.$swal({
                        icon: 'success',
                        title: '¡Éxito!',
                        text: 'Gravado actualizado con éxito.',
