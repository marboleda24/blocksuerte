...
<head>
    ...
    <title>@lang('Monitor de trabajos')</title>
            ...
            <div class="flex items-center my-2 -mx-2">
                ...
                <div class="px-2 w-1/4">
                    ...
                    <select name="type" id="filter_show" class="w-full p-2 bg-gray-200 border-2 border-gray-300 rounded">
                        <option @if($filters['type'] === 'all') selected @endif value="all">@lang('Todo')</option>
                        <option @if($filters['type'] === 'running') selected @endif value="running">@lang('En progreso')</option>
                        <option @if($filters['type'] === 'failed') selected @endif value="failed">@lang('Fallidos')</option>
                        <option @if($filters['type'] === 'succeeded') selected @endif value="succeeded">@lang('Finalizados')</option>
                    </select>
                </div>
    ...
    <div class="overflow-x-auto shadow-lg">
        ...
        <table class="w-full rounded whitespace-no-wrap">
            ...
            <thead class="bg-gray-200">
                ...
                <tr>
                    <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Estado')</th>
                    <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Job')</th>
                    <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Detalles')</th>
                    ...
                    <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Progreso')</th>
                    <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Duracion')</th>
                    <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Iniciado')</th>
                    ...
                    @if(config('queue-monitor.ui.allow_deletion'))
                        <th class="px-4 py-3 font-medium text-left text-xs text-gray-600 uppercase border-b border-gray-200">@lang('Accion')</th>
                    @endif
                </tr>
                ...
                @forelse($jobs as $job)
                    ...
                    <tr class="font-sm leading-relaxed">
                        ...
                        <td class="p-4 text-gray-800 text-sm leading-5 border-b border-gray-200">
                            ...
                            <div class="text-xs">
                                <span class="text-gray-600 font-medium">@lang('Cola'):</span>
                                <span class="font-semibold">{{ $job->queue }}</span>
                            </div>

                            <div class="text-xs">
                                <span class="text-gray-600 font-medium">@lang('Intento(s)'):</span>
                    ...
                    <td colspan="100" class="px-6 py-4 text-gray-700 font-sm border-t-2 border-gray-200">
                        ...
                        <div class="flex justify-between">
                            ...
                            <div>
                                @lang('Mostrando')
                                @if($jobs->total() > 0)
                                    <span class="font-medium">{{ $jobs->firstItem() }}</span> @lang('al')
                                    <span class="font-medium">{{ $jobs->lastItem() }}</span> @lang('de')
                                @endif
                                <span class="font-medium">{{ $jobs->total() }}</span> @lang('registros')
                            </div>
